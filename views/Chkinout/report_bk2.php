...
while (strtotime($strStartDate) <= strtotime($strEndDate)){
    ...
    if($DayOfWeek == 0)  // 0 = Sunday, 6 = Saturday;
    {
        ...
        $sqlday_off = "SELECT DISTINCT SUBSTR(checktime,1,11) AS date,c.userid
            FROM scan_chkinout as c
            WHERE c.userid = '$userid'
            AND DATE_FORMAT(c.checktime,'%Y-%m-%d') = '$strStartDate';";

        $result_dayoff = Yii::$app->db->createCommand($sqlday_off)->queryAll();
        $cout_dayoff = count($result_dayoff);
...
//     //$noscan = ($count_leave - $dayoff);

//     //จำนวนวันรวม ที่ไม่มีการแสกนนิ้วมือ
//     $sumscandate = $intWorkDay - ($count_leave - $noscan);

//     //$sumscandate = $intWorkDay - $noscan;
...
$count_leaveweek = Yii::$app->db->createCommand($sqlweek)->queryAll();
foreach ($count_leaveweek as $r) {
    # code...
    $dateweek = $r['date'];

    $DayOfWeek1 = date("w", strtotime($dateweek));
        if($DayOfWeek1 == 0 || $DayOfWeek1 == 6){
            $dayoffweek++;
    }
...
//วันลาจริงที่ไม่ใช่ วันหยุด

$sumweek_off = $sumleaveall - $dayoffweek;


//วันที่แสกน - วันที่แสกน - วันหยุดแต่แสกนลายนิ้วมือ
$daynoscan = ($count_leave - $dayoff)-$sumweek_off;



// //แสกน >  วันทำงาน
if($daynoscan > $intWorkDay){

    //แสกนนิ้วเกินจากวันทำงาน
    $noscan = $daynoscan - $intWorkDay;
...
//แสกน < วันทำงาน 
    ...
    //จำนวนวันรวม ที่ไม่มีการแสกนนิ้วมือ
    $sumscandate = $intWorkDay - ($daynoscan - $noscan);
...
<td style='font-size: 16px;' align='left' colspan='2'><b>" . "วันทั้งหมด  ".$intTotalDay."  วัน   วันทำงาน  ".$intWorkDay. "  วัน   วันหยุดงาน  ".$intHoliday."  วัน</b><b> วันหยุดนักขัตฤกษ์  ".$intPublicHoliday." วัน</b><b> รวมวันหยุดทั้งหมด ".($intHoliday+$intPublicHoliday)." วัน </b></td>
     <td style='font-size: 16px;' align='right' colspan='5'>สรุป มาสาย ".$count_daydate." ครั้ง ไม่แสกนลายนิ้วมือ "
     .$sumscandate."  ครั้ง ลงชื่อ ......................................................................( หัวหน้าแผนก )</td>"
. "</tr>
